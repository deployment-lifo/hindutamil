...
EPUBJS.Reader = function(bookPath, _options) {
  ...
  this.settings =settings = EPUBJS.core.defaults(_options || {}, {
    bookPath : bookPath,
    restore : false,
    reload : false,
    bookmarks : undefined,
  ...
  rendition.on('locationChanged', function (e,i) {
    console.log('after this',this);
    console.log('locationChanged');
    console.log(reader.settings.bookKey);

    localStorage[reader.settings.bookKey+'previousLocationCfi']= e.start;
    reader.settings.previousLocationCfi = e.start;
    if(!localStorage) {
      return false;
    }

    localStorage[reader.settings.bookKey]=(JSON.stringify(reader.settings));
    console.log('localStorage[this.settings.bookKey]',localStorage[reader.settings.bookKey]);
  });
console.log('localStorage[this.settings.bookKey]',localStorage[this.settings.bookKey]);
console.log('this.settings.previousLocationCfi',this.settings.previousLocationCfi);
  ...
  book.ready.then(function () {
    ...
    /*this.rendition.on("swipeleft", this.rendition.next(this));
    this.rendition.on("swiperight", this.rendition.next(this));*/
...
EPUBJS.Reader.prototype.isSaved = function(bookPath) {
  ...
  console.log('this.settings.bookKey',this.settings.bookKey);
  storedSettings = localStorage[this.settings.bookKey];
...
EPUBJS.Reader.prototype.applySavedSettings = function() {
  ...
  try {
    stored = JSON.parse(localStorage[this.settings.bookKey]);
...
EPUBJS.Reader.prototype.saveSettings = function(){
  ...
  localStorage[this.settings.bookKey]=(JSON.stringify(this.settings));
};
  ...
  */
  $slider.on("click", function () {
    if(reader.sidebarOpen) {
      ...
      $slider.find('i').addClass('fa-bars');
      $slider.find('i').removeClass('fa-arrow-left');
      $('.epub-toolbar').slideDown();
      //$slider.addClass("icon-menu");
      //$slider.removeClass("icon-right");
    } else {
      reader.SidebarController.show();
      $slider.find('i').addClass('fa-arrow-left');
      $slider.find('i').removeClass('fa-bars');
      $('.epub-toolbar').slideUp();
