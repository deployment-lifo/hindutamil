...
(function ($) {
	...
	*/
	var defaults = {
		...
		loaded: null,
		changed: null,
		loadingHeight: window.innerHeight+300,
		loadingWidth: window.innerWidth+200,
		loadingHTML: "Loading..."
	};
		...
		*/
		...
		function initDom() {
			...
			if (options.showToolbar) {
				...
				/*$element.find(".pdf-toolbar").html(
					...
					+ '<div class="pdf-button"><button class="pdf-prev viewer-controls"><i class="fa fa-angle-left"></i></button></div>'
				 	+ '<div class="pdf-button"><span class="pdf-page-count"></span></div>'
				 	+ '<div class="pdf-button"><button class="pdf-next viewer-controls"><i class="fa fa-angle-right"></i></button></div>'
				 	+ (options.disableZoom? '':'<div class="pdf-button"><button class="pdf-zoomin viewer-controls"><i class="fa fa-search-plus"></i></button></div>'
				 		+ '<div class="pdf-button"><button class="pdf-zoomout viewer-controls"><i class="fa fa-search-minus"></i></button></div>')
					+ '<div class="pdf-button showDownLoadButton" style="display:none;"><button class="viewer-controls saveFileOffline"><i class="fa fa-download"></i></button></div>'
				 	+ '<div class="pdf-button close-pdf-viewer"><a href="#" class="back viewer-controls"><i class="fa fa-times"></i></a></div>'
				 	);*/
				 $(".pdf-toolbar").html('<div class="toolbar-inner"><a class="pdf-prev  link"><i class="fa fa-angle-left"></i></a>'
				 	+ '<a class="pdf-page-count link"></a>'
				 	+ '<a class="pdf-next  link"><i class="fa fa-angle-right"></i></a>'
				 	+ (options.disableZoom? '':'<a class="pdf-zoomin viewer-controls link"><i class="fa fa-search-plus"></i></a>'
				 		+ '<a class="pdf-zoomout link"><i class="fa fa-search-minus"></i></a>')
					+ '<a class=" saveFileOffline link showDownLoadButton" style="display:none;"><i class="fa fa-download"></i></a>'
				 	+ '<a href="#" class="back  link"><i class="fa fa-times"></i></a></div>'
				 	);
				
				$(".pdf-toolbar .pdf-title").on("click", function() {
					goto(1);
				});
				$(".pdf-toolbar .pdf-prev").on("click", function() {
					goto(pageNum-1);
				});
				$(".pdf-toolbar .pdf-next").on("click", function() {
					goto(pageNum+1);
				});
			}
			...
			if (!options.disableZoom) {
				...
				$drag.parent().on('mousewheel.focal', function( e ) {
					e.preventDefault();
					//console.log('mousewheel',e.originalEvent);
				...
				var el = document.querySelector('div.pdf-pinch');
      			var pinchZoomObj=new PinchZoom.default(el, {});
      			console.log(pinchZoomObj);
				/*$drag.parent().on('touchstart.focal', function( e ) {
					//onTouch(e);
					var owner = panController.getOwner();
					console.log(owner);
				});*/
				
				/*$drag.parent().on('touchend.focal', function( e ) {
					
				});*/
				
				if (options.showToolbar) {
					$(".pdf-toolbar .pdf-zoomin").on("click", function() {
						console.log('pdf-zoomin');
						zoomIn();
					});
					$(".pdf-toolbar .pdf-zoomout").on("click", function() {
						console.log('pdf-zoomout');
						zoomOut();
					});
				}
		...
		function onTouch(e) {
			...
			if (e.originalEvent.touches.length === 1) {
				return handleSingleFingerTouch(e, e.originalEvent.touches[0]);
			} else if (e.originalEvent.touches.length === 2) {
				// handleTouchMove() will care about pinch zoom.
				pinchZoomLength = getPinchZoomLength(e.originalEvent.touches[0], e.originalEvent.touches[1]);
				multiTouch = true;
		...
		*/
		function renderPage () {
			...
			pdfDoc.getPage(pageNumRendering).then(function(page) {
				...
				var scaleDevice=window.innerWidth/canvas.width;
				if(window.innerWidth>700)
					scaleDevice=scaleDevice-0.1;
				$(".pdf-canvas").css("transform", "scale("+(scaleDevice)+") translate(-50%,-50%)").css("transform-origin", "top left").css('position','absolute').css('top','48%').css('left','50%');
		...
		function updatePageCount() {
			if (state == EMPTY || state == INIT) return;
			$(".pdf-page-count").html(pageNum + " / " + totalPages);
		}
