...
var androidVersion = "2.1.0";
var iosVersion = "2.1.0";
				...
				for(var i = 0;i < res.data.length;i++) {
					...
					if(menu.length > 0) {
						...
						if(data[i].mid != 4 && data[i].mid != 5 && data[i].mid != 99 && data[i].mid != 2 && data[i].mid != 7) {
							console.log(data[i].menu_name_tamil);
							content += '<a data-tab="#tab-'+(tab)+'" data-id="'+(tab)+'" data-tab-id="'+i+'" data-mid="'+data[i].mid+'" data-cid="'+data[i].cid+'" id="getOtherPages" class="tab-link">'+data[i].menu_name_tamil+'</a>';
...
function appendHomePage(data,val) {
	console.log('appendHomePage');
	googleHomeDfpAd(function(){
			...
			if(data[i].type == 'type10') {
				...
				if(post.length > 0) {
					for(var j = 0;j < post.length;j++) {
						if(post[j].type == 'in') {
							content += '<div id="div-gpt-ad-1605624326531-0"><script async src=\"https://securepubads.g.doubleclick.net/tag/js/gpt.js\"></script>\r\n<script>\r\n  window.googletag = window.googletag || {cmd: []};\r\n  googletag.cmd.push(function() {\r\n    googletag.defineSlot("/21697178033/Apps_HT_Digital_Subs_728x320", [[320, 50], [728, 90]], "div-gpt-ad-1605624284210-0").defineSizeMapping(mapping1).addService(googletag.pubads());\r\n    googletag.pubads().enableSingleRequest();\r\n    googletag.enableServices();\r\n  });\r\n</script></div>';
						}
					}
				}
			}
...
function refreshGoogleAd() {
	...
	googletag.cmd = googletag.cmd || [];
	googletag.cmd.push(function() {
      googletag.display('div-gpt-ad-1605624326531-0');
    });
}
...
function triggerHomeDetails(page) {	
	...
	var data = {key:accessToken,type:'home',page:scrollPageNo,v:1.3};
				...
				for(var i = 0;i < data.length;i++) {
					...
					if(menu.length > 0) {
						...
						if(data[i].mid =='2' && data[i].cid == 0) {
							...
							content += '<li class="panel-close"><a href="'+menuLink+'" class="item-link item-content moveToTab" data-mid="'+(typeof(data[i].mid)!='undefined'?data[i].mid:'')+'" data-cid="'+(typeof(data[i].cid)!='undefined'?data[i].cid:'')+'"><div class="item-media"><i class="'+data[i].menu_image+'"></i></div><div class="item-inner"><div class="item-title">'+data[i].menu_name_tamil+'</div></div></a></li>';	
						} else {
							content += '<li class="panel-close"><a href="'+menuLink+'" class="item-link item-content '+(menuLink=='javascript:void(0)'?'noContent':'')+'" data-mid="'+(typeof(data[i].mid)!='undefined'?data[i].mid:'')+'" data-cid="'+(typeof(data[i].cid)!='undefined'?data[i].cid:'')+'"><div class="item-media"><i class="'+data[i].menu_image+'"></i></div><div class="item-inner"><div class="item-title">'+data[i].menu_name_tamil+'</div></div></a></li>';	
						}
					}
...
function refreshAds(refresh_slots) { 
	console.log('refresh_slots',refresh_slots);
...
console.log(moment("2021-03-18").unix());
function createGoogleAds() {
	...
	var bannerType = 'FULL_BANNER';
	 ...
	 if(moment().unix()>='1615228200' && (device.model.includes('iPad')))
	 {
		if(typeof(AdMob) != 'undefined') {
			AdMob.createBanner( {
			adId:admobid.banner, 
...
function googleHomeDfpAd(callback) {
	console.log('googleHomeDfpAd');
	window.adTags = {};
	/*window.googletag = window.googletag || {cmd: []};  
