...
var androidVersion = "2.1.1";
var iosVersion = "2.1.3";
		...
		success:function(res) {
			...
			if(res.msg == "success") {
				...
				if(curPageSplit[0]=='pages/news-detail.html'){
					...
					if(res.data.user_access.epaper==1){
	...
	if(data != null) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			if(data[i].type == 'type10') {
				...
				if(post.length > 0) {
					if(post[0].type == 'in') {
						/*if(typeof($(post[0].tag).closest('div').attr('id')) != 'undefined') {
		...
		$.ajax({
			...
			success:function(res) {
				if(res.msg == 'success'){ 
					...
					firstcontent = $(data.content).find("p").andSelf().filter("p:first").first().text();
					...
					}else if(data.content_type=='paid' && login.user_id!='' && login.user_access.epaper!=1 ){
					...
					if(data != null) {
						...
						$('.page-current .newsDetailDescription').html(''+paid_content+'<div id=\'div-gpt-ad-1605624597553-0\' class=\'text-center\' ></div><div style="overflow-x:auto;"> '+data.content+'</div>');
